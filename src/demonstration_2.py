...
def find_judge(N, trust):
    ...
    for judge in range(1,N+1):
        if judge not in trust_d:
            for t_set in trust_d.values():
                if judge not in t_set:
                    return -1
            return judge
...
def find_judge_instructor(N, trust):
    """
    Inputs:
    N -> int
    trust -> List[List[int]]
...
print(find_judge_instructor(4, [[1,3],[1,4],[2,3],[2,4],[4,3]]))
